...
During combat, totem-slot values can become secret. A totem that was readable before combat keeps its last known icon and counts down from its readable start and duration until expiry. A successful player `UNIT_SPELLCAST_SUCCEEDED` event for a learned totem shows its icon without a fabricated timer; `PLAYER_TOTEM_UPDATE` clears a later removal, and out-of-combat readable state replaces the observation. This uses only player spellcast and totem-update events, not the blocked combat-log event. Fresh casts that cannot be identified from the spellbook remain unknown.
